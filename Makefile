...
coverage:
	coverage run --source=apilityio setup.py develop test
	coverage report
	coverage html
...
install-testpypi:
	pip install --pre -i https://testpypi.python.org/pypi apilityio

install-pypi:
	pip install apilityio

install-develop:
	PYPI=$(PYPI) python setup.py develop

uninstall:
	pip uninstall apilityio
